...
public class ReusableMethods {
    ...
    public static void bransOgrenciSayisiYazdir(Map<Integer, String> sinifListMap) {
        ...
        for (String each:ValuesCollections) {
            ...
            // bransın bransOgrSayiMap inde key olarak daha onceden eklenip eklenmedğinikontrol etmeliyiz
    ...
    public static void bransOgrencisayisiYazdir(Map<Integer, String> sinifListMap) {
        ...
        // brans=bransdakiOgrenciSayisi
        Map<String,Integer> bransOgrSayiMap= new TreeMap<>();// harf sırasın a gore olsun dedim o yuzden treemap yaptım

        Collection<String> valuesCollection= sinifListMap.values();// value leri almak için collection olusturdum
        String[] valueArr;// parcalara boluyorum ya
        String brans;
        Integer bransdakiOgrSayisi;
        for (String each: valuesCollection// tek tek ıcınden alabılmek ıcın foor each olusturdum
        ) {
            valueArr=each.split(", ");
